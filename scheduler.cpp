...
UBaseType_t maxResNums = 2;  //Set number of resources
Res* localRes[2];
Res* R[2];
UBaseType_t CPlist[2];  //Array to store ceiling priorities of resources
static void prvAssignDynamicPriority(Res* res, TaskHandle_t* CallingTask);  

/*Initialization function for Resources*/
/*Takes number of resources & ceiling priorities for resources in an array*/
void vInitResources(UBaseType_t maxResNum, UBaseType_t xCP[]) 
{
	for(int i=0; i<maxResNum; i++)	//Loop for initializing all resources
	{
		R[i] = (Res*)pvPortMalloc(sizeof(Res*));
		localRes[i] = R[i];	 //Create a local copy of resources. Can be eliminated in future
		CPlist[i] = xCP[i];  //Set ceiling priority array via user input 
		// PRINTF("CPlist  : %d \n", CPlist[i]);

		if(R[i] != NULL)
		{
			R[i]->xCeilingPriority = CPlist[i];		//Set ceiling priority of resource. **Need to debug why value is changing for OCPP. Using CPlist directly for now
			R[i]->xSemaphore = xSemaphoreCreateMutex();	//Create semaphore for each resource
			R[i]->xInUse = pdFALSE; //Set resource inUse to false initially
			// PRINTF("in use : %d \n", R[i]->xInUse);

			if( R[i]->xSemaphore != NULL )
			{
				Serial.print(i);
...
// Function for Blocking a task in OCPP implementatioin
void prvBlockTaskOnScheduler(Res* res, TaskHandle_t* CallingTask)
{
	Serial.println("************Blocked**********l");	//For delay. Need to figure out other alternative 
	
	//portMAX_DELAY Blocks task indefinitely on the semaphore
	if(xSemaphoreTake(res->xSemaphore , portMAX_DELAY ) == pdTRUE) 
	{
		res->xInUse = pdTRUE;			//Set Resource xInUse flag to true
		res->xUserTask = CallingTask;	//Store Task handle of Semaphore holder on the resource
		/*Semaphore is obtained*/
		Serial.print("Resource obtained after blocked ----- !!");
		Serial.print(" - ");
	...
	{
		/*Semaphore is not obtained*/
		Serial.print("Resource not obtained - Time Exceeded!!");
		Serial.print(" - ");
		Serial.println(xTaskGetTickCount());
		Serial.flush();
	}	
...
//Function to obtain the resource. Called by high priority tasks
void prvObtainresource(Res* res, TaskHandle_t* CallingTask)
{
	Serial.println("--------OBTAINED-----------");	//For delay
	Serial.flush();
	//portMAX_DELAY Blocks task indefinitely on the smeaphore
	if(xSemaphoreTake(res->xSemaphore , portMAX_DELAY )== pdTRUE)	//
	{
		res->xInUse = pdTRUE;	//Set Resource xInUse flag to true
		res->xUserTask = CallingTask;	//Store Task handle of Semaphore holder on the resource
		/*Semaphore is obtained*/
		Serial.print("Resource obtained !!");
		Serial.print(" - ");
		Serial.println(xTaskGetTickCount());
		Serial.flush();
		// PRINTF("Used: %d \n", res->xInUse);
...
// Function used to inherit the higher priority and then block that task on the semaphore
void inheritAndBlock(Res* res, TaskHandle_t* highPriorityTask)
{
	Serial.println("*************Inherit & block************");	//delay
	Serial.flush();
	taskENTER_CRITICAL();
	res->xUserTaskPriority = uxTaskPriorityGet(*(res->xUserTask));  //Stores original priority on the resource before inheriting
	vTaskPrioritySet(*(res->xUserTask), uxTaskPriorityGet(*highPriorityTask));	//Sets User task priority to higher priority
	taskEXIT_CRITICAL();
	prvBlockTaskOnScheduler(res, highPriorityTask);  //Block task on that resource indefinitely

}

// Function used to pre-empt the user task by the higher priority task and force user task to give up the semaphore
void prvPreemptTask(Res* res, TaskHandle_t* PreemtingTask)
{
	Serial.println("///////////Pre-empt//////////");

	taskENTER_CRITICAL();
	xSemaphoreGive(res->xSemaphore); //pre-empt the user task by forcing task to give up semaphore
	res->xInUse = pdFALSE;	//Set Resource xInUse flag to false
	Serial.println("Resource pre-empted !!");
	Serial.flush();
	taskEXIT_CRITICAL();

	prvObtainresource(res, PreemtingTask);	//Obtain the freed resource 
}

//Function called by tasks to request a resource. Parameters: Resource, task handle of caller, block time 
void vSchedulerResourceWait(Res* res, TaskHandle_t* xCallingTask, TickType_t xBlockTime)
{
	#if (schedCP_PROTOCOL == schedCP_PROTOCOL_ICPP)	//ICPP implementation
		Serial.println("---Wait() called----");
		if(res->xSemaphore != NULL)
		{
			if(xSemaphoreTake(res->xSemaphore , xBlockTime) == pdTRUE)	//Request the semaphore till WCET
			{
				//When resource is obtained, set the dynamic priority of task to Ceiling priority of resource
				prvAssignDynamicPriority(res, xCallingTask);	
				res->xInUse = pdTRUE;	//Set Resource xInUse flag to true
				/*Semaphore is obtained*/
				Serial.println("Resource obtained !!");
				Serial.flush();
			...
			{
				/*Semaphore is not obtained*/
				Serial.println("Resource not obtained - WCET exceeded !!");
				Serial.flush();
			}	
		}
	...
	#if (schedCP_PROTOCOL == schedCP_PROTOCOL_OCPP)	//OCPP implementation

		Serial.println("------------Wait()------------------");
		Serial.flush();	

		//Resource to be accessed is in use
		if(pdTRUE == res->xInUse)	
		{
			Serial.println("+++++++++ In use +++++++++++++");
			Serial.flush();	
			//If resource is in use & if calling task priority <= user task priority; block the task on semaphore
			if((pdTRUE == (res->xInUse)) && (uxTaskPriorityGet(*(res->xUserTask)) >= uxTaskPriorityGet(*xCallingTask)))
			{
				Serial.println("------block block----------");
				Serial.flush();
				prvBlockTaskOnScheduler(res, xCallingTask);  //Block task on that resource indefinitely
				
			}
			//If resource is in use & if calling task priority > user task priority; inherit the high priority & block the HP task
			else if((pdTRUE == (res->xInUse)) && (uxTaskPriorityGet(*(res->xUserTask)) < uxTaskPriorityGet(*xCallingTask)))
			{
				Serial.println("------inheritance block----------");
				// Serial.flush();
				inheritAndBlock(res, xCallingTask);		//Inherit the priority & block the task on semaphore
			}
	
		}
		else //Resource to be accessed is not in use
		{
			Serial.println("-----------------Not in use------------------");
			Serial.flush();	

			bool flag = false;
			for(int i = 0; i<maxResNums; i++) //Loop through all resources except self resource
			{
				// PRINTF("CP : %d \n", CPlist[i]);
				if((localRes[i] != res) && (pdTRUE == (localRes[i]->xInUse)))  //If other resource is in use and is not self resource
				{
					if(CPlist[i] < uxTaskPriorityGet(*xCallingTask))  //If ceiling priority of locked resource is lesser than calling task's priority, pre-empt
					{
						Serial.println("--------------Pre-empted-------------------");
						prvPreemptTask(res, xCallingTask);	//Pre-empt user task
						flag = true;
						
					}
					else  //If ceiling priority of locked resource is greater than calling task's priority, block the task
					{
						prvBlockTaskOnScheduler(res, xCallingTask);  //Block task on that resource indefinitely
						flag = true;
					}
			...
			if(flag == false)
			{
				prvObtainresource(res, xCallingTask);  //If the task is the first to arrive; obtain the resource
			}
...
//Function to be called when resource is to be released
void vSchedulerResourceSignal(Res* res, TaskHandle_t* CallingTask)
{
	#if (schedCP_PROTOCOL == schedCP_PROTOCOL_ICPP)	//ICPP implementation
		Serial.print("Resource released !!");
		Serial.print(" - ");
		Serial.println(xTaskGetTickCount());
		Serial.flush();
		...
		xSemaphoreGive(res->xSemaphore);	//Give up the semaphore
		vTaskPrioritySet(*CallingTask, res->xUserTaskPriority);	//Reset the priority to the base priority saved on the resource
		res->xInUse = pdFALSE;  //Set resource inuse flag to false
		/*Semaphore released*/
		...
		xSemaphoreGive(res->xSemaphore);	//Give up the semaphore
		vTaskPrioritySet(*CallingTask, res->xUserTaskPriority);	//Reset the priority to the previous priority stored on the resource
		res->xInUse = pdFALSE;  //Set resource inuse flag to false
		/*Semaphore released*/
		taskEXIT_CRITICAL();
	#endif
...
// Funtion to be called during ICPP implementation to set the semaphore holder's priority to ceiling priority of resource
static void prvAssignDynamicPriority(Res* res, TaskHandle_t* CallingTask)
{
	#if (schedCP_PROTOCOL == schedCP_PROTOCOL_ICPP)

		res->xUserTaskPriority = uxTaskPriorityGet(*CallingTask);

		if(res->xCeilingPriority > uxTaskPriorityGet(*CallingTask))
		{
			// vTaskPrioritySet(*(res->xUserTask), res->xCeilingPriority);
			vTaskPrioritySet(*CallingTask, res->xCeilingPriority);
			Serial.println("TP = CP set!!");
			Serial.flush();
		}

		res->xUserTask = CallingTask;
